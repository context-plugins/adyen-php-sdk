...
```php
use AdyenLib\Models\Builders\StoredValueStatusChangeResponseBuilder;
use AdyenLib\Models\Builders\AmountBuilder;
use AdyenLib\Models\ResultCode3Enum;

$storedValueStatusChangeResponse = StoredValueStatusChangeResponseBuilder::init()
    ->authCode('authCode2')
    ->currentBalance(
        AmountBuilder::init(
            'currency2',
    ...
    ->resultCode(ResultCode3Enum::ERROR)
    ->build();
```
