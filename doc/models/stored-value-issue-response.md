...
| `paymentMethod` | `?array<string,string>` | Optional | The collection that contains the type of the payment method and its specific information if available | getPaymentMethod(): ?array | setPaymentMethod(?array paymentMethod): void |
...
```php
use AdyenLib\Models\Builders\StoredValueIssueResponseBuilder;
use AdyenLib\Models\Builders\AmountBuilder;

$storedValueIssueResponse = StoredValueIssueResponseBuilder::init()
    ->authCode('authCode8')
    ->currentBalance(
        AmountBuilder::init(
            'currency2',
