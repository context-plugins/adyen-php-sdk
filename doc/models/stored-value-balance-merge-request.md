...
|  --- | --- | --- | --- | --- | --- |
| `amount` | [`?Amount`](../../doc/models/amount.md) | Optional | The amount information for the transaction. | getAmount(): ?Amount | setAmount(?Amount amount): void |
...
| `sourcePaymentMethod` | `array<string,string>` | Required | The collection that contains the source payment method and its specific information if available. Note that type should not be included since it is inferred from the (target) payment method | getSourcePaymentMethod(): array | setSourcePaymentMethod(array sourcePaymentMethod): void |
...
```php
use AdyenLib\Models\Builders\StoredValueBalanceMergeRequestBuilder;
use AdyenLib\Models\Builders\AmountBuilder;
use AdyenLib\Models\ShopperInteractionEnum;

$storedValueBalanceMergeRequest = StoredValueBalanceMergeRequestBuilder::init(
    'merchantAccount0',
    [
        'key0' => 'paymentMethod0',
        'key1' => 'paymentMethod9',
