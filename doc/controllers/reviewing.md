...
```php
$reviewingApi = $client->getReviewingApi();
```
...
```php
function postConfirmThirdParty(?ModifyRequest $body = null): ModifyResponse
```
...
```php
$body = ModifyRequestBuilder::init(
...
try {
    $result = $reviewingApi->postConfirmThirdParty($body);
    echo 'ModifyResponse:';
    var_dump($result);
...
```php
function postDeclineThirdParty(?ModifyRequest $body = null): ModifyResponse
```
...
try {
    $result = $reviewingApi->postDeclineThirdParty($body);
    echo 'ModifyResponse:';
    var_dump($result);
} catch (ServiceErrorException $exp) {
    echo 'Caught ServiceErrorException:', $exp;
