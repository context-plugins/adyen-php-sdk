...
| `md` | `string` | Required | The payment session identifier returned by the card issuer. | getMd(): string | setMd(string md): void |
...
| `paResponse` | `string` | Required | Payment authorisation response returned by the card issuer. The `paResponse` field holds the PaRes value received from the card issuer. | getPaResponse(): string | setPaResponse(string paResponse): void |
...
```php
use AdyenLib\Models\Builders\PaymentRequest3dBuilder;
...
$paymentRequest3d = PaymentRequest3dBuilder::init(
    'md0',
    'merchantAccount6',
    'paResponse0'
)
