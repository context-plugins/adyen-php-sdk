...
```php
$businesslinesApi = $client->getBusinesslinesApi();
```
...
```php
function postBusinessLines(?BusinessLineInfo $body = null): BusinessLine
```
...
```php
$body = BusinessLineInfoBuilder::init(
    '4531',
...
try {
    $result = $businessLinesApi->postBusinessLines($body);
    echo 'BusinessLine:';
    var_dump($result);
...
```php
function getBusinessLinesId(string $id): BusinessLine
```
...
| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The unique identifier of the business line. |
...
try {
    $result = $businessLinesApi->getBusinessLinesId($id);
    echo 'BusinessLine:';
    var_dump($result);
} catch (ServiceErrorError1Exception $exp) {
    echo 'Caught ServiceErrorError1Exception:', $exp;
...
```php
function deleteBusinessLinesId(string $id): void
```
...
| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The unique identifier of the business line to be deleted. |
...
try {
    $businessLinesApi->deleteBusinessLinesId($id);
} catch (ServiceErrorError1Exception $exp) {
    echo 'Caught ServiceErrorError1Exception:', $exp;
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
...
```php
function patchBusinessLinesId(string $id, ?BusinessLineInfoUpdate $body = null): BusinessLine
```
...
$body = BusinessLineInfoUpdateBuilder::init()
    ->industryCode('55')
    ->webData(
        [
            WebDataBuilder::init()
...
try {
    $result = $businessLinesApi->patchBusinessLinesId(
        $id,
        $body
    );
    echo 'BusinessLine:';
