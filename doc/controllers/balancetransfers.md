...
```php
$balancetransfersApi = $client->getBalancetransfersApi();
```
...
```php
function postBalanceTransfers(BalanceTransferRequest $body): BalanceTransferResponse
```
...
```php
$body = BalanceTransferRequestBuilder::init(
    Amount17Builder::init(
        'EUR',
        3000
    )->build(),
    'SOURCE_MERCHANT_ACCOUNT',
    'DESTINATION_MERCHANT_ACCOUNT',
    BalanceTransferType2Enum::ADJUSTMENT
)
...
try {
    $result = $balanceTransfersApi->postBalanceTransfers($body);
    echo 'BalanceTransferResponse:';
    var_dump($result);
