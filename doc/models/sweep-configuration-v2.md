...
| `id` | `string` | Required, Read-only | The unique identifier of the sweep. | getId(): string | setId(string id): void |
...
| `schedule` | [`SweepSchedule1`](../../doc/models/sweep-schedule-1.md) | Required | The schedule when the `triggerAmount` is evaluated. If the balance meets the threshold, funds are pushed out of or pulled in to the balance account. | getSchedule(): SweepSchedule1 | setSchedule(SweepSchedule1 schedule): void |
...
$sweepConfigurationV2 = SweepConfigurationV2Builder::init(
    ...
    ->category(Category1Enum::INTERNAL)
