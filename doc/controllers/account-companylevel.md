...
```php
function getCompanies(?int $pageNumber = null, ?int $pageSize = null): ListCompanyResponse
```
...
try {
    $result = $accountCompanyLevelApi->getCompanies();
...
```php
function getCompaniesCompanyId(string $companyId): Company2
```
...
try {
    $result = $accountCompanyLevelApi->getCompaniesCompanyId($companyId);
    echo 'Company2:';
    var_dump($result);
} catch (RestServiceErrorException $exp) {
    echo 'Caught RestServiceErrorException:', $exp;
...
try {
    $result = $accountCompanyLevelApi->getCompaniesCompanyIdMerchants($companyId);
