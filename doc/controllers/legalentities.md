...
```php
$legalentitiesApi = $client->getLegalentitiesApi();
```
...
```php
$body = LegalEntityInfoRequiredTypeBuilder::init(
...
try {
    $result = $legalEntitiesApi->postLegalEntities(
        null,
        $body
    );
    echo 'LegalEntity3:';
...
```php
function getLegalEntitiesId(string $id): LegalEntity3
```
...
try {
    $result = $legalEntitiesApi->getLegalEntitiesId($id);
    echo 'LegalEntity3:';
    var_dump($result);
} catch (ServiceErrorError1Exception $exp) {
    echo 'Caught ServiceErrorError1Exception:', $exp;
...
|  --- | --- | --- | --- |
| `id` | `string` | Template, Required | The unique identifier of the legal entity. |
| `xRequestedVerificationCode` | `?string` | Header, Optional | Use the requested verification code 0_0001 to resolve any suberrors associated with the legal entity. Requested verification codes can only be used in your test environment. |
...
$body = LegalEntityInfoBuilder::init()
    ->type(Type182Enum::INDIVIDUAL)
    ->build();

$legalEntitiesApi = $client->getLegalEntitiesApi();

try {
    $result = $legalEntitiesApi->patchLegalEntitiesId(
        $id,
        null,
        $body
    );
...
```php
function getLegalEntitiesIdBusinessLines(string $id): BusinessLines
```
...
try {
    $result = $legalEntitiesApi->getLegalEntitiesIdBusinessLines($id);
    echo 'BusinessLines:';
    var_dump($result);
} catch (ServiceErrorError1Exception $exp) {
    echo 'Caught ServiceErrorError1Exception:', $exp;
...
```php
function postLegalEntitiesIdCheckVerificationErrors(string $id): VerificationErrors
```
...
try {
    $result = $legalEntitiesApi->postLegalEntitiesIdCheckVerificationErrors($id);
...
```php
function postLegalEntitiesIdConfirmDataReview(string $id): DataReviewConfirmationResponse
```
...
try {
    $result = $legalEntitiesApi->postLegalEntitiesIdConfirmDataReview($id);
    echo 'DataReviewConfirmationResponse:';
    var_dump($result);
} catch (ServiceErrorError1Exception $exp) {
    echo 'Caught ServiceErrorError1Exception:', $exp;
} catch (ApiException $exp) {
...
```php
function postLegalEntitiesIdRequestPeriodicReview(string $id)
```
...
try {
    $result = $legalEntitiesApi->postLegalEntitiesIdRequestPeriodicReview($id);
    echo 'mixed:';
    var_dump($result);
} catch (ApiException $exp) {
    echo 'Caught:', $exp;
