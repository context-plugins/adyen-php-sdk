...
```php
function postPaymentLinks(?string $idempotencyKey = null, ?PaymentLinkRequest $body = null): PaymentLinkResponse
```
...
try {
    $result = $paymentLinksApi->postPaymentLinks(
        null,
        $body
    );
...
```php
function getPaymentLinksLinkId(string $linkId): PaymentLinkResponse
```
...
try {
    $result = $paymentLinksApi->getPaymentLinksLinkId($linkId);
    echo 'PaymentLinkResponse:';
    var_dump($result);
} catch (ServiceErrorException $exp) {
    echo 'Caught ServiceErrorException:', $exp;
...
```php
function patchPaymentLinksLinkId(string $linkId, ?UpdatePaymentLinkRequest $body = null): PaymentLinkResponse
```
...
try {
    $result = $paymentLinksApi->patchPaymentLinksLinkId(
        $linkId,
        $body
    );
    echo 'PaymentLinkResponse:';
