...
```php
$platformApi = $client->getPlatformApi();
```
...
```php
function getBalancePlatformsId(string $id): BalancePlatform
```
...
try {
    $result = $platformApi->getBalancePlatformsId($id);
    echo 'BalancePlatform:';
    var_dump($result);
...
```php
function getBalancePlatformsIdAccountHolders(
    string $id,
    ?int $offset = null,
    ?int $limit = null
...
try {
    $result = $platformApi->getBalancePlatformsIdAccountHolders($id);
    echo 'PaginatedAccountHoldersResponse:';
    var_dump($result);
} catch (RestServiceErrorException $exp) {
    echo 'Caught RestServiceErrorException:', $exp;
} catch (ApiException $exp) {
...
```php
function getBalancePlatformsIdTransactionRules(string $id): TransactionRulesResponse
```
...
try {
    $result = $platformApi->getBalancePlatformsIdTransactionRules($id);
